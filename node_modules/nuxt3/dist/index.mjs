...
import { resolveFiles, useNuxt, defineNuxtModule, addVitePlugin, addWebpackPlugin, addPlugin, addTemplate, isNuxt3, resolveAlias, addPluginTemplate, nuxtCtx, addComponent, installModule, loadNuxtConfig, templateUtils, normalizeTemplate, compileTemplate, findPath, normalizePlugin } from '@nuxt/kit';
...
import { getNitroContext, createDevServer, resolveMiddleware as resolveMiddleware$1, build as build$1, prepare, generate, scanMiddleware, writeTypes, wpfs } from '@nuxt/nitro';
...
const version = "3.0.0-27404660.0f9bcbf";
...
function initNitro(nuxt) {
  ...
  nuxt.hook("modules:done", async () => {
    ...
    const { middleware, legacyMiddleware } = await resolveMiddleware$1(nuxt);
...
const cssTemplate = {
  filename: "css.mjs",
  getContents: (ctx) => ctx.nuxt.options.css.map((i) => genImport(i.src || i)).join("\n")
};
