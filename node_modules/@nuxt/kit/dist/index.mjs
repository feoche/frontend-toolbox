import { parse, basename, resolve, normalize, join, isAbsolute, dirname, relative, extname } from 'pathe';
...
async function findPath(paths, opts) {
...
async function loadNuxtConfig(opts) {
  ...
  const { config: nuxtConfig, configFile, layers } = await loadConfig({
    ...
    rcFile: ".nuxtrc",
    dotenv: typeof opts.dotenv === "undefined" ? {} : opts.dotenv,
    globalRc: true,
...
function addAutoImport(_autoImports) {
